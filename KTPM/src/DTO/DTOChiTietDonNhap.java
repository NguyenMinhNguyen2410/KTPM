/*
 * To change this license header, choose License Headers in Project Properties.
 * To change this template file, choose Tools | Templates
 * and open the template in the editor.
 */
package DTO;

/**
 *
 * @author nguye
 */
public class DTOChiTietDonNhap {
    private int IDDonNhap,IDSanPham;
    private float DonGia;
    private String IDKhung,Mau;

    public DTOChiTietDonNhap() {
    }

    public DTOChiTietDonNhap(int IDDonNhap, int IDSanPham, float DonGia, String IDKhung, String Mau) {
        this.IDDonNhap = IDDonNhap;
        this.IDSanPham = IDSanPham;
        this.DonGia = DonGia;
        this.IDKhung = IDKhung;
        this.Mau = Mau;
    }

    public int getIDDonNhap() {
        return IDDonNhap;
    }

    public void setIDDonNhap(int IDDonNhap) {
        this.IDDonNhap = IDDonNhap;
    }

    public int getIDSanPham() {
        return IDSanPham;
    }

    public void setIDSanPham(int IDSanPham) {
        this.IDSanPham = IDSanPham;
    }

    public float getDonGia() {
        return DonGia;
    }

    public void setDonGia(float DonGia) {
        this.DonGia = DonGia;
    }

    public String getIDKhung() {
        return IDKhung;
    }

    public void setIDKhung(String IDKhung) {
        this.IDKhung = IDKhung;
    }

    public String getMau() {
        return Mau;
    }

    public void setMau(String Mau) {
        this.Mau = Mau;
    }
    
}
